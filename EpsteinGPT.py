...
def log_run_step(run_id: int, step: int, loss: float, ce: float, reg: float, ckpt_path: Optional[str]):
    conn = get_db()
    with _db_lock, conn:
        conn.execute(
            "INSERT OR REPLACE INTO steps(run_id, step, loss, ce, reg, timestamp) VALUES(?,?,?,?,?,?)",
...
def get_last_good_checkpoint() -> Optional[Tuple[int, str]]:
    ...
    with _db_lock, conn:
        row = conn.execute(
            "SELECT id, last_ckpt_path FROM runs WHERE status='success' AND last_ckpt_path IS NOT NULL "
...
# =========================================================
# 8. ORIGINAL Epistemic Functions
# =========================================================
...
def fetch_open_source_docs() -> List[Path]:
    ...
    for archive in CFG.public_archives:
        try:
            ...
            log_event("WARN", "ingestion", {"msg": "archive_failed", "archive": archive, "error": str(e)})
            continue
    _flush_metadata_buffer()
    return new_files
...
class TrainingManager:
    ...
    def _safe_save_checkpoint(self, ckpt_path: Path, state: Dict) -> bool:
        try:
            ...
            log_event("ERROR", "checkpoint", {"msg": "save_failed", "path": str(ckpt_path), "error": str(e)})
            return False
            ...
            for epoch in range(CFG.epochs):
                for input_ids, attention_mask in dataloader:
                    ...
                    with self.training_lock:
                        ...
                        metadatas = sample_source_metadatas(CFG.num_roots)
                        A_prior = torch.rand(batch_size, seq_len, device=CFG.device)
                        root_probs = torch.rand(batch_size, seq_len, CFG.num_roots, device=CFG.device)
                        corr_matrix = torch.eye(CFG.num_roots, device=CFG.device).unsqueeze(0).repeat(batch_size,1,1)
    ...
    def train_streaming_epoch(self):
        ...
        try:
            for input_ids, attention_mask in dataloader:
                ...
                with self.training_lock:
                    self.model.train()
                    input_ids = input_ids.to(CFG.device, non_blocking=(CFG.device=="cuda"))
                    attention_mask = attention_mask.to(CFG.device, non_blocking=(CFG.device=="cuda"))
                    ...
                    if self.amp_enabled:
                        try:
                            ...
                            log_event("ERROR", "training_amp", {"msg": "amp_step_failed", "error": str(e)})
                            optimizer.zero_grad(set_to_none=True)
                    else:
                        loss.backward()
                        torch.nn.utils.clip_grad_norm_(self.model.parameters(), CFG.max_grad_norm)
            ...
            log_event("ERROR", "training_amp", {"msg": "train_streaming_epoch_error", "error": str(e)})
            if self.run_id is not None:
                log_run_end(self.run_id, "interrupted")
...
class Supervisor:
    ...
    def _check_disk_usage(self):
        ...
        if ratio > CFG.max_disk_usage_ratio:
            ...
            ckpts = sorted(Path(CFG.checkpoint_dir).glob("*.pt"), key=lambda p: p.stat().st_mtime)
    ...
    def _check_gpu_memory(self):
        if CFG.device == "cuda":
            ...
            log_event(
                "INFO",
                "supervisor",
                {"msg": "gpu_memory", "allocated": int(mem_alloc), "reserved": int(mem_reserved)},
            )
...
# =========================================================
# 13. Multimodal Encoders (real implementations)
# =========================================================
...
class VideoEncoder:
    ...
    def encode_path(self, path: Path) -> np.ndarray:
        """
        Sample ~1 frame per second and average CLIP embeddings.
        """
        ...
        fps = max(int(fps) if fps > 0 else 1, 1)
        frame_interval = fps
...
def print_status():
    conn = get_db()
    with _db_lock, conn:
        run = conn.execute("SELECT * FROM runs ORDER BY start_time DESC LIMIT 1").fetchone()
        step = conn.execute("SELECT * FROM steps ORDER BY timestamp DESC LIMIT 1").fetchone()
        docs_count = conn.execute("SELECT COUNT(*) AS c FROM docs").fetchone()["c"]
    print("=== STATUS ===")
    if run:
        print(f"Last run id: {run['id']} status={run['status']} last_step={run['last_step']}")
    else:
        print("No runs recorded.")
    if step:
        print(f"Last step: {step['step']} loss={step['loss']:.4f} ce={step['ce']:.4f} reg={step['reg']:.4f}")
    else:
...
def print_runs(limit: int = 5):
    ...
    for r in rows:
        print(
            ...
            f"start={time.ctime(r['start_time'])} end={time.ctime(r['end_time']) if r['end_time'] else 'N/A'}"
        )
...
# =========================================================
# 15. Unified Chat API (OpenAI-style) + ChatEngine
# =========================================================
...
class ChatRequest(BaseModel):
    ...
    top_p: float = 0.9
    max_tokens: int = 256
    stream: bool = False  # streaming not implemented in this single-file version
...
class ChatEngine:
    ...
    def _download_to_temp(self, url: str, suffix: str) -> Path:
        ...
        tmp = Path(f"./tmp_{hashlib.md5(url.encode('utf-8')).hexdigest()}{suffix}")
        tmp.write_bytes(resp.content)
        return tmp

    def _collect_modal_embeddings(self, msg: ChatMessage) -> Tuple[Optional[np.ndarray], Optional[np.ndarray], Optional[np.ndarray]]:
        img_emb = None
        audio_emb = None
        video_emb = None
        ...
        for part in msg.content:
            if part.type == "image_url" and part.image_url and IMAGE_ENCODER is not None:
                ...
                path = self._download_to_temp(url, ".png")
                img_emb = IMAGE_ENCODER.encode_path(path)
                path.unlink(missing_ok=True)
            elif part.type == "audio_url" and part.audio_url and AUDIO_ENCODER is not None:
                url = part.audio_url.url
                path = self._download_to_temp(url, ".wav")
                audio_emb, _ = AUDIO_ENCODER.encode_path(path)
                path.unlink(missing_ok=True)
            elif part.type == "video_url" and part.video_url and VIDEO_ENCODER is not None:
                url = part.video_url.url
                path = self._download_to_temp(url, ".mp4")
                video_emb = VIDEO_ENCODER.encode_path(path)
                path.unlink(missing_ok=True)

        return img_emb, audio_emb, video_emb

    def _flatten_message_text(self, msg: ChatMessage) -> str:
        parts = [p.text for p in msg.content if p.type == "text" and p.text]
        return " ".join(parts).strip()

    def _build_prompt(self, history: List[ChatMessage], new_msg: ChatMessage) -> str:
        lines = []
        for m in history:
            txt = self._flatten_message_text(m)
            if txt:
                lines.append(f"{m.role.upper()}: {txt}")
        new_txt = self._flatten_message_text(new_msg)
        if new_txt:
            lines.append(f"{new_msg.role.upper()}: {new_txt}")
    ...
    def chat(self, req: ChatRequest, session_id: str = "default") -> ChatResponse:
        ...
        new_msg = req.messages[-1]
        if new_msg.role not in ("user", "system"):
            raise HTTPException(status_code=400, detail="Last message must be 'user' or 'system'")

        session_history = self._get_session(session_id)
        if history_messages:
            session_history.extend(history_messages)

        prompt = self._build_prompt(session_history, new_msg)
        img_emb, audio_emb, video_emb = self._collect_modal_embeddings(new_msg)
        ...
        with self.trainer.training_lock:
            ...
            with torch.inference_mode():
                ...
                if MM_PREFIX is not None and (img_emb is not None or audio_emb is not None or video_emb is not None):
                    tok_embs = self.model.transformer.wte(input_ids)
                    fused_embs = MM_PREFIX(tok_embs, img_emb=img_emb, audio_emb=audio_emb, video_emb=video_emb)
...
def chat_completions(req: ChatRequest = Body(...)):
    ...
    # In a real deployment you might use a header or param for session_id.
    return chat_engine.chat(req, session_id="default")
